...
public class SettingsActivity extends android.preference.PreferenceActivity
		implements OnNetworkResultAvailableListener {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		this.menu.findItem(R.id.menu_progress).setVisible(false)
				.setActionView(progressBar);
		progressBar.setPadding(5, 0, 5, 0);

		boolean goToInfoPref = getIntent().getBooleanExtra("goToInfoPref",
				false);
		...
		if (goToInfoPref) {
			...
			for (Header header : headers) {
				if (header.fragment.equals(bhApp.getPackageName()
						+ ".activity.SettingsActivity$InfoFragment")) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case ChangePasswordActivity.MODE_CHANGE_ONLINE_PASS:
			if (resultCode == 1) {
				...
				PrefNetThread changePass = new PrefNetThread(this, bhApp,
						netManager);
				changePass
						.execute(
								AuthentificationSecure.SERVER_PASS_CHANGE,
								String.valueOf(Authentification.NETRESULT_ID_PASS_CHANGE),
								"h="
										+ bhApp.authentification
												.getPassChangeHash(newPass),
	...
	public boolean onResult(int requestId, String resultString) {
		...
		if (requestId == Authentification.NETRESULT_ID_PASS_CHANGE) {
			...
			if (matcher.find()) {
				...
				String errorMsg = ErrorHandler.getErrorString(bhApp, requestId,
						error);
			...
			if (resultString.equals("<SUCCESS>")) {
				Toast.makeText(bhApp,
						getString(R.string.str_Preferences_changePass_success),
	...
	public static class InfoFragment extends PreferenceFragment {
			...
			issueTrackerPref
					.setOnPreferenceClickListener(new OnPreferenceClickListener() {
						...
						public boolean onPreferenceClick(Preference preference) {
							Intent trackerIntent = new Intent(
									Intent.ACTION_VIEW,
									Uri.parse("http://bluehunter.maks-dev.com/issues"));
		...
		private void initializeStaticPrefs() {
			...
			try {
				infoPref.setSummary(getActivity().getPackageManager()
				...
				showNotificationPref
						.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
							...
							public boolean onPreferenceChange(
								...
								bhApp.mainActivity.alterNotification(newValue
										.equals(true));
				...
				deleteRemember
						.setOnPreferenceClickListener(new OnPreferenceClickListener() {
							...
							public boolean onPreferenceClick(
								...
								PreferenceManager.setPref(bhApp,
										"pref_enableBT_remember", 0);
	...
	public static class ProfileFragment extends PreferenceFragment {
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			if (bhApp.loginManager != null && bhApp.mainActivity != null
					&& !bhApp.mainActivity.passSet
					&& bhApp.loginManager.getLoginState()) {
				changeOnlinePass
						.setTitle(R.string.str_Preferences_changePass_new_title);
				changeOnlinePass
						.setSummary(R.string.str_Preferences_changePass_new_sum);
		...
		private void registerListeners() {
			...
			changeOnlinePass
					.setOnPreferenceClickListener(new OnPreferenceClickListener() {
						...
						public boolean onPreferenceClick(Preference preference) {
							...
							parametersBundle
									.putInt("mode",
											ChangePasswordActivity.MODE_CHANGE_ONLINE_PASS);
							parametersBundle.putBoolean("passSet",
									bhApp.mainActivity.passSet);
			...
			changeLoginPass
					.setOnPreferenceClickListener(new OnPreferenceClickListener() {
						...
						public boolean onPreferenceClick(Preference preference) {
							...
							parametersBundle
									.putInt("mode",
											ChangePasswordActivity.MODE_CHANGE_LOGIN_PASS);
							intent.putExtras(parametersBundle);
			...
			bhApp.authentification
					.setOnLoginChangeListener(new OnLoginChangeListener() {
			...
			syncInterval
					.setOnPreferenceClickListener(new OnPreferenceClickListener() {
						...
						public boolean onPreferenceClick(Preference preference) {
							...
							builder.setPositiveButton("Save",
									new OnClickListener() {
										...
										public void onClick(
											...
											NumberPicker numberPicker = (NumberPicker) ((AlertDialog) dialog)
							...
							builder.setView(LayoutInflater.from(
									builder.getContext()).inflate(
									R.layout.dlg_sync_interval, null));

							AlertDialog alertDialog = builder.show();

							NumberPicker numberPicker = (NumberPicker) alertDialog
									.findViewById(R.id.numberPicker1);
							numberPicker.setMinValue(1);
							numberPicker.setMaxValue(100);
							numberPicker.setWrapSelectorWheel(false);
							numberPicker.setValue(PreferenceManager.getPref(
									getActivity(), "pref_syncInterval", 5));
			...
			rebuildDB
					.setOnPreferenceClickListener(new OnPreferenceClickListener() {
						...
						public boolean onPreferenceClick(Preference preference) {
							...
							int result = new DatabaseManager(bhApp)
									.rebuildDatabase();

							if (result == 0) {
								Toast.makeText(
										getActivity(),
										"Successfully rebuilt database. App will now restart.",
										Toast.LENGTH_LONG).show();
							} else if (result < 0) {
								Toast.makeText(
										getActivity(),
										(-result)
												+ " devices could not be rebuild. App will now restart.",
										Toast.LENGTH_LONG).show();
							} else if (result == 1001) {
								Toast.makeText(getActivity(),
										"Database could not be rebuilt.",
										Toast.LENGTH_LONG).show();
								return true;
							}
		...
		private void checkList() {
			...
			if (curRunningThreads.size() == 0) {
				if (!prefActivity.isDestroyed()) {
					MenuItem progressBar = prefActivity.menu
							.findItem(R.id.menu_progress);
	...
	public class PrefNetThread extends AsyncTask<String, Integer, String> {
		...
		public PrefNetThread(SettingsActivity preferenceActivity,
				BlueHunter app, PrefNetManager networkMananger) {
		...
		protected String doInBackground(String... params) {
			...
			if (remoteFile.startsWith("https"))
				https = true;
			...
			try {
				...
				for (int i = 2; i < params.length; i++) {
					...
					postValues.add(new BasicNameValuePair(matcher.group(1),
							matcher.group(2)));
				}
		...
		protected void onPreExecute() {
			...
			if (!preferenceActivity.isDestroyed()) {
				MenuItem progressBar = preferenceActivity.menu
						.findItem(R.id.menu_progress);
